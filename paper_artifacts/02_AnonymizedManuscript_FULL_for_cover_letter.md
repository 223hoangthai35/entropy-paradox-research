<!--
COVER-LETTER REFERENCE COPY (synced to v9, 2026-05-07)

This file is a synced snapshot of the main manuscript
(02_AnonymizedManuscript_v3_4.md), maintained as source material for the
JFDS submission cover letter and editorial narrative use.

Distil the following from this manuscript when drafting the cover letter:

  1. Principal Contribution 1 — Entropy Paradox direction heterogeneity
     extended to 8 markets with three-method convergence
     (Cliff δ + causal forest DML + earlier 3-market observational study).
     Decisive results: SPX Paradox (ATE = +3.48 [+0.92, +6.04]);
                       BTC Inverted (ATE = −9.34 [−18.07, −0.61]).

  2. Principal Contribution 2 — Cross-market H–RPS magnitude coupling
     (tier-based ρ = 0.927; η²-corrected ρ = 0.964; cascade composite
     ρ mean = 0.847) under a transparently archived cascade RPS framework
     with comprehensive sensitivity validation.

  3. Principal Contribution 3 — Heterogeneous-effects DML methodology
     (causal forest + Purged K-Fold per López de Prado 2018 §7.3 +
     filtered Schmitt-trigger labels) for financial regime classification
     with overlapping forward-looking targets.

  4. Principal Contribution 4 — Schmitt-trigger generalisation supporting
     all three findings (label-flicker artifacts removed empirically per
     the DML methodology study).

Methodological signals to highlight in the cover letter:

  - Frozen public archive of hypothesis specifications and analysis decisions (anonymised for double-blind review; tag in title page)
  - Byte-for-byte regression assertions against the frozen analysis archive
  - Cascade RPS verification process — the KOSPI single update is one
    cascade outcome rather than a special-case correction
  - Three-method convergence on H1 strengthens the empirical case
  - 75–99% epistemic / 1–25% aleatoric variance decomposition directs the
    methodological roadmap toward primary-source RPS infrastructure rather
    than statistical-power extension

Suggested reviewers (independent academics in entropy/finance and ML/finance):

  - Brouty / Garcin (entropy and the market efficiency programme)
  - Bucci / Ciciretti (regime detection on realised covariances)
  - Athey or Wager affiliate (causal forest methodology)
  - Cohen / Maneejuk (post-COVID structural-break analysis)

Target journal: Journal of Finance and Data Science (KeAi/Elsevier),
Q1, IF 3.9, APC roughly $550–700.
Submission audience: data-science scholarship application portfolio.

This file mirrors the main manuscript byte-for-byte after the sync. It is
not a separately edited document. Edits should be made in v3_4.md and
then re-synced into this file. The HTML comment block above is the only
addition.
-->
