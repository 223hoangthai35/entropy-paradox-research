...
**No composite fallback**: Earlier prototypes shipped with a deterministic entropy-aggregate "Tri-Vector Composite Risk" path that activated when GARCH-X was unavailable. v7.1 removes it. With ≥ 120 trading days of entropy features always available in the supported markets, GARCH-X is the single source of truth for the risk gauge; if the engine is genuinely unavailable (missing `arch` dependency, fit failure on degenerate data) the dashboard surfaces the error explicitly rather than silently falling back to a less-rigorous score.
...
LAYER 5 — VERDICT MATRIX (Regime-Aware Risk Classification)
  ...
  No composite-score fallback in v7.1 — GARCH-X failure is surfaced explicitly.
